...
#pragma warning restore CS0168
            ...
            foreach (var url in initialUrls)
            {
                ...
                catch (Exception e)
                {
                    ...
                    if (Logger.IsWarnEnabled)
                        Logger.Warn($"Error occurred while attempting to fetch the Cluster Tag for {url} in {nameof(SingleTopologyUpdateAsync)}", e);
                }
        ...
        private HttpCache.ReleaseCacheItem GetFromCache<TResult>(JsonOperationContext context, RavenCommand<TResult> command, SessionInfo sessionInfo, string url, out string cachedChangeVector, out BlittableJsonReaderObject cachedValue)
        {
            ...
            if (command.CanCache && command.CanReadFromCache && command.IsReadRequest && command.ResponseType == RavenCommandResponseType.Object)
            {
...
#pragma warning restore SYSLIB0014 // Type or member is obsolete
                ...
                {
                    if (Logger.IsWarnEnabled)
                        Logger.Warn($"Failed to set the connection limit for url: {url}", e);
                }
            }
        }
